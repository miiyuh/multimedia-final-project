...
const Navbar = () => {
  ...
  const { timeRemaining } = useCaseState();
          ...
          <div className="flex items-center">
            <Link href="/">
              <a className="flex items-center">
                <span className="text-primary text-2xl font-bold font-mono">Ransom<span className="text-secondary">Track</span></span>
                <span className="ml-2 px-2 py-1 text-xs font-mono bg-dark-100 rounded-md text-light-300">Sim v1.0</span>
              </a>
            </Link>
          </div>
          ...
          <div className="hidden md:flex space-x-6 items-center">
            {navLinks.map((link) => (
              <Link key={link.path} href={link.path}>
                <a className={`text-light-200 hover:text-primary transition font-medium ${location === link.path ? 'text-primary' : ''}`}>
                  {link.label}
                </a>
              </Link>
            ))}
            ...
            <Sheet>
              ...
              <SheetContent className="bg-dark-200 border-dark-100">
                <div className="flex flex-col space-y-4 mt-8">
                  {navLinks.map((link) => (
                    <Link key={link.path} href={link.path}>
                      <a className={`text-xl font-medium ${location === link.path ? 'text-primary' : 'text-light-200'} hover:text-primary transition py-2`}>
                        {link.label}
                      </a>
                    </Link>
                  ))}
