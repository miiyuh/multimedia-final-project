...
import { clickSound } from "@/utils/sound"; // Import the sound
...
const Footer = () => {
  ...
  // Play sound when buttons with id="play-sound" are clicked
  const handlePlaySound = (event: React.MouseEvent<HTMLButtonElement>) => {
    clickSound.play(); // Play the click sound
    if (event.currentTarget.id === "play-sound" && saveCase) {
      saveCase(); // Call saveCase if it's the save button
    }
  };
    ...
    <footer className="bg-dark-200 border-t border-dark-100 py-8">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="flex flex-col md:flex-row md:space-x-8 items-center">
            <div className="mb-4 md:mb-0">
              <div className="text-light-200 font-bold mb-2">
            ...
            <div className="flex space-x-4">
              <Button
                id="play-sound"
                onClick={handlePlaySound} // Attach the sound handler
                variant="outline"
                size="icon"
        ...
        <div className="border-t border-dark-100 mt-6 pt-6 text-center text-light-300 text-sm">
          &copy; 2025 RansomTrack Cybercrime Investigation Simulation | Educational Use Only
        </div>
      </div>
    </footer>
  );
